...
const Orders = () => {
  ...
  useEffect(() => {
    fetch(`https://shielded-wave-20768.herokuapp.com/orders`)
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
        setLoading(false);
  ...
  const handleStatusChange = (id, status) => {
    ...
    fetch("https://shielded-wave-20768.herokuapp.com/updateOrderStatus", {
      method: "put",
  ...
  const deletion = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://shielded-wave-20768.herokuapp.com/placeorder/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
